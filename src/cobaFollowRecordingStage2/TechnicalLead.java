...
public class TechnicalLead extends TechnicalEmployee {
    ...
    public boolean requestBonus(Employee e, double bonus) {
        ...
        */
        if (this.getManager().approveBonus(bonus)) {
    ...
    public String getTeamStatus() {
        ...
        */
        String result = this.employeeStatus() + "\n";
        if (team.size() == 0) {
            result += " and no direct reports yet";
        } else {
            for (SoftwareEngineer e : team) {
                result += " and " + e.employeeStatus();
            }
        }
        return result;
    }
    public SoftwareEngineer[] getTeam() {
        return null;
    }
    public void setAccountant(Accountant e) {
    }
}
